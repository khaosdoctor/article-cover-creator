...
const gracefulShutdown = async () => {
  log(`Shutting down...`)
  await browser.close()
  await caches.delete('blog-articles')
  Deno.exit(0)
}
...
router.get('/blog/articles', async (ctx) => {
  ...
  if (cachedImage) {
    Deno.stdout.write(new TextEncoder().encode(`Cache hit for ${ctx.request.url}\n`))
    const image = await cachedImage.arrayBuffer()
    return sendImageResponse(ctx, new Uint8Array(image) as Buffer)
  }

  Deno.stdout.write(new TextEncoder().encode(`Cache miss for ${ctx.request.url}, saving...\n\n`))
  ...
  await articleCache.put(ctx.request.url, new Response(image, { headers: { 'Content-Type': 'image/png' } }))
